...
describe('HttpExceptionFilter — standard error envelope', () => {
  ...
  it('keeps a specific, human-written 4xx message', () => {
    const { body } = run(new ConflictException('This leave request has already been decided'));
  ...
  it('preserves an explicit machine-readable code (teaching-context isolation)', () => {
    const { status, body } = run(
      new ForbiddenException({
        ...
        message: 'This is an Academy batch. Switch to that academy profile to manage it.',
      }),
  ...
  it('turns class-validator arrays into VALIDATION_FAILED + details, message stays a string', () => {
    const { status, body } = run(
      new BadRequestException(['capacity must be a positive number', 'title should not be empty']),
    );
    expect(status).toBe(400);
    expect(body.code).toBe(ErrorCode.VALIDATION_FAILED);
    expect(body.details).toEqual(['capacity must be a positive number', 'title should not be empty']);
    expect(typeof body.message).toBe('string');
    expect(body.message).toContain('capacity must be a positive number');
  });

  it('maps throttling to RATE_LIMITED with safe copy', () => {
    const { status, body } = run(new HttpException('ThrottlerException: Too Many Requests', 429));
  ...
  describe('server errors never leak internals', () => {
    ...
    it('a 5xx HttpException without a code hides its message', () => {
      const { body } = run(new InternalServerErrorException('Razorpay client not initialized'));
    ...
    it('a 5xx HttpException the thrower vouched for (explicit code) keeps its message', () => {
      ...
      expect(body.message).toBe('Could not send the code by email — try again shortly.');
    });
  ...
  it('preserves extra keys a thrower attached (health check flags)', () => {
    const { status, body } = run(
      new ServiceUnavailableException({ status: 'error', database: 'down', redis: 'up' }),
    );
    expect(status).toBe(503);
    expect(body).toMatchObject({ status: 'error', database: 'down', redis: 'up', statusCode: 503 });
  });

  describe('structured logging', () => {
    it('logs the REAL error with requestId, caller and context at error level for 5xx', () => {
      run(new Error('relation "secret_table" does not exist'), { 'x-teaching-context': 'academy:abc' });
