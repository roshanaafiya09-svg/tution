...
import { CardSkeleton, ErrorState, EmptyState, Field, Input, PageHeader, Select } from '@/components/ui';
import { ParentCard, ParentSectionHeader } from '@/components/parent';

export default function ParentFindATeacherPage() {
      ...
      {results?.curated && (
        <ParentCard className="mb-6 flex items-start gap-3 border-info/20 bg-info-bg dark:border-info/25 dark:bg-info/10">
          ...
          <p className="text-sm text-info dark:text-info-dark">
            We&apos;re still growing in this area — here&apos;s a curated list of teachers rather than a filtered
            search. Full search opens up once there are more teachers and students nearby.
          </p>
        </ParentCard>
      )}

      <ParentSectionHeader title="Teachers" />
      ...
      {loadError ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {filtered.results.map((tutor) => (
            <Link key={tutor.tutorId} href={`/parent/find-a-teacher/${tutor.slug}`}>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-2">
                      <p className="font-medium text-neutral-900 dark:text-neutral-50">{tutor.displayName ?? 'Teacher'}</p>
                    ...
                    {tutor.headline && <p className="text-sm text-neutral-500 dark:text-neutral-400">{tutor.headline}</p>}
