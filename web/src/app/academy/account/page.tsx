...
/** Academy's "Account" page — login identity plus DPDP data-principal
 *  rights (export/deletion), same shape and endpoints as Teacher's
 *  /dashboard/profile. Deliberately does not duplicate Academy Profile's
 *  editable fields (name, description, ...) — those stay on Academy
 *  Profile per the Navigation, Routing & UX Update spec. */
export default function AcademyAccountPage() {
  ...
  async function exportData() {
    ...
    try {
      ...
      link.href = url;
      link.download = `scholar-academy-data-export-${new Date().toISOString().slice(0, 10)}.json`;
      link.click();
  ...
  return (
    <div>
      <AcademyPageIntro
        ...
        title="Account"
        description="Your login details, plus data export and account deletion controls."
      />
      ...
      <div className="mt-8">
        {hasAcademy === false ? (
          ...
          <>
            <AcademyCard className="max-w-2xl">
            ...
            <AcademyCard className="mt-6 max-w-2xl border-error/20 dark:border-error/25">
