...
export interface AnnouncementWithBatch extends Announcement {
  batch_title: string;
...
/** Today/This week/Earlier bucketed announcement feed — shared by the
 *  global Announcements page (all batches) and each batch workspace's
 *  Announcements tab (one batch's announcements already pre-filtered by the
 *  caller). */
export function AnnouncementsList({ announcements }: { announcements: AnnouncementWithBatch[] }) {
          ...
          <div className="space-y-3">
            {group.items.map((a) => (
              <AcademicCard key={a.id}>
                <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
                  <p className="flex items-center gap-2 text-sm font-medium text-neutral-900 dark:text-neutral-50">
                    ...
                    {a.tutor_display_name && (
                      <span className="font-normal text-neutral-400 dark:text-neutral-500">· {a.tutor_display_name}</span>
                    )}
                  </p>
                ...
                <p className="mt-3 font-display text-lg leading-snug text-neutral-800 dark:text-neutral-100">{a.body}</p>
              </AcademicCard>
            ))}
          </div>
        </section>
