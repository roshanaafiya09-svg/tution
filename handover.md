...
## 7. Teacher Profile + Find a Teacher (post-blueprint feature)

Not part of the original BLUEPRINT 2.md phases (§1) — a separate feature request, built and verified after the §3 redesign. Adds a teacher-managed public profile and a teacher-discovery search to the Student and Parent dashboards, **extending** the existing marketplace/discovery system (§1 Phase 4) rather than building a parallel one — same discovery search, same `profiles_tutor`/`tutor_subjects`/`reviews`/`tutor_locations` tables, same booking flow, same notifications, same storage provider.

**What shipped:**
- Tutor Dashboard → **Teacher Profile** (`/dashboard/teacher-profile`, its own nav item under Business → Teaching profile — separate from the dashboard home page and from account settings at `/dashboard/profile`): Set Up → Save → Edit flow for photo, name, headline, bio, years experience, qualifications, languages, teaching mode (online/offline/both), methodology, achievements, certifications, fee note. Verified badge, rating, students-taught, and available batches are shown read-only — no manual text field for any of them.
- Student Dashboard → **Find a Teacher** (`/student/find-a-teacher` + `/student/find-a-teacher/[slug]`, under More → Learning) and Parent Dashboard → **Find a Teacher** (`/parent/find-a-teacher` + `/parent/find-a-teacher/[slug]`, a standalone top-level nav item — not nested under Marketplace or any dropdown): search bar, filters (subject/curriculum/grade reused from the existing `/discover` page, plus new teaching-mode/language/experience/fee/rating filters), teacher cards, full profile view (subjects/rates, available batches with live seat counts, reviews). Students get **Book a session** (links straight into the existing, untouched `/book/[slug]` flow) + **Contact Teacher**; parents get **Contact Teacher** only — confirmed live that no booking CTA renders for parents, since the app has no parent-facing booking capability anywhere.
- **Avatar upload** reuses the exact same `StorageProvider` (Supabase Storage in prod, local-disk in dev) as course materials — extracted into a shared `backend/src/common/storage/` module both `DeliveryModule` and `IdentityModule` import, rather than a second storage registration. JPG/PNG/WebP accepted, 5MB cap, validated both client- and server-side; replace/remove both work.
- **Students-taught and available batches are computed live, not stored.** `ProofOfTeachingService.countStudentsTaught()` unions distinct students from batch enrollments and completed 1:1 bookings — the same "verified session, two sources" pattern `ReviewsService` already used. `BatchesRepository.listOpenWithSeatsForTutor()` is a single grouped query (batches left-joined to active enrollments, no N+1), exposed via `GET /batches/me/open`.
- **Contact Teacher** persists a lead (new `teacher_contact_requests` table, migration `0029`) and fires the existing `NotificationsService.notify()` — no new notification mechanism. Visible to the tutor as a new section on the existing `/dashboard/marketplace` page, with New/read state and mark-as-read.
- Backend: one migration (`0029`) — 8 nullable columns added to `profiles_tutor` (photo object key, qualifications, languages, teaching mode, methodology, achievements, certifications, fee note) plus the `teacher_contact_requests` table. No other new tables. `GET /marketplace/discovery/tutors` extended with the new filters; the density gate (§1 Phase 4) is unchanged.

**Real bugs found and fixed while building/verifying this:**
- **Cross-origin avatar image silently blocked in local dev** — `@fastify/helmet`'s default `Cross-Origin-Resource-Policy: same-origin` blocked the browser from loading an `<img>` served from the API origin (this is the first feature to embed a photo inline rather than fetch/download it). Fixed by setting `Cross-Origin-Resource-Policy: cross-origin` on just the one dev-only `materials/local-download/:objectKey` route, not the global Helmet config — doesn't affect production, where Supabase Storage serves avatars from its own domain.
- **Teacher Profile edit form overflowed horizontally at 375px** — the avatar-upload row's text column had no shrink/wrap constraint. Fixed with `flex-wrap` + `min-w-0 flex-1`.
- **The "4.5+ stars" rating filter 400'd on every single use** — `SearchTutorsDto.minRating` was `@IsInt()`, but the filter UI's own dropdown option sends `4.5`. Relaxed to `@IsNumber({ maxDecimalPlaces: 1 })`.
- **The "Max fee" filter's rupee→paise conversion (`× 100`) hit IEEE-754 floating-point imprecision** for many ordinary two-decimal rupee amounts (e.g. `8.03 → 802.9999999999999`), which also failed `@IsInt()`. Both of these surfaced identically as a generic "Could not load teachers" error, since the frontend's blanket `.catch()` couldn't distinguish a validation 400 from a real network failure — root-caused by directly `curl`-reproducing the exact query strings the frontend sends (not by touching the density gate or discovery ranking, both confirmed unrelated). Fixed with `Math.round()`.

**Verified:** real (non-impersonated, OTP-authenticated) tutor/student/parent sessions for every mutation — profile save/edit/refresh-persistence, photo upload (JPG/PNG/WebP + invalid-type/oversized rejection, both client- and server-side) and replace/remove, Contact Teacher creating a real DB row + notification, and mark-as-read (impersonated sessions were confirmed to correctly 403 on mutations, per the existing security guard from §6.5 — not a bug, the intended behavior). 375px/768px/desktop + dark mode checked on all 5 new routes. Backend `tsc --noEmit` + scoped ESLint clean; frontend `tsc --noEmit` + scoped ESLint + full `next build` clean.

**Known limitation:** the dev DB is below the discovery density gate (25 tutors/250 students — §1 Phase 4), so filters return the existing curated-pool fallback there rather than a live filtered search; this is pre-existing, unrelated gate behavior that this feature didn't change.

## 8. Find an Academy (post-blueprint feature)

Not part of BLUEPRINT 2.md (§1) — built 2026-08-13, one session before §9 below. Adds an academy-as-entity discovery layer parallel to Find a Teacher (§7): academies are a separate org entity from tutors, connected via a new `academy_memberships` table — a tutor can be independent-only, academy-only, or belong to multiple academies simultaneously, and no duplicate teacher-profile data is ever created per academy.

**What shipped:**
- Migration `0030`: `academies`, `academy_locations` (PostGIS, mirrors `tutor_locations`), `academy_memberships`, `academy_photos`, `academy_reviews` (a rating system deliberately kept separate from teacher `reviews` — an academy's trust signal never mixes with an individual teacher's), `academy_contact_requests`. Subjects/grades an academy offers are derived at read time from active memberships → `tutor_subjects`, never a duplicate table.
- Backend: three modules, not two — `academy-memberships/` is a leaf module (repository only, no controller) imported independently by both `academies/` and `academy-reviews/`, avoiding a Nest module cycle between them.
- Student/Parent dashboards → **Find an Academy** (list + profile pages), mirroring Find a Teacher's UI pattern. Academy profile page's "Our Teachers" links to the *existing* Teacher Profile route (`/{role}/find-a-teacher/{slug}`) — no second teacher-profile UI.
- **Bidirectional link**: the public tutor page and the tutor's own Teacher Profile both gained a "Teaching under 🏫 X" badge sourced from `academy_memberships`.
- Its own density gate (`ACADEMY_DENSITY_GATE = { academies: 3 }` in `academies.service.ts`), separate from Find a Teacher's — academies are far rarer than tutors, so reusing that gate would have kept academy search permanently in curated-fallback mode.
- **Deliberate scope boundary at the time**: no self-serve academy-admin role existed yet — all academy data (create/verify/photos/memberships) was superadmin-only (`/admin/academies/*`), and `academy_contact_requests` was persist-only (no notification, since no academy-owner user existed to notify). **This gap is now closed — see §9.**

**Verified (at the time):** backend `tsc`/eslint/migration up-down-up/boot check; frontend `tsc`/eslint/`next build`; live browser walkthrough with a real test academy + membership, both directions of the bidirectional link, mobile layout.

## 9. Academy Dashboard + Teacher-to-Academy join workflow (post-blueprint feature)

Built 2026-08-14 — closes the self-serve-role gap §8 explicitly left open. Gives an Academy its own account/role/dashboard, and lets a Teacher search academies, request to join, and get accepted/rejected, while remaining a fully independent teacher account throughout.

**Hard invariant, enforced everywhere and verified live**: Teacher and Academy accounts/roles/data never merge. The new `academy` role is only ever granted to an academy's own owner user, never to a tutor. Joining/leaving an academy only ever writes to the membership tables below — never to `users`/`profiles_tutor`/`tutor_subjects`/batches/bookings/reviews. Live-verified: removing a teacher from an academy left `GET /marketplace/discovery/tutors/{slug}` completely unchanged except `academies: []` — no data loss anywhere else.

**What shipped:**
- Migration `0031`: adds `'academy'` to the `user_roles` role check (additive — roles are a many-to-many join table, not an enum column, so this needed no schema restructuring); `academies.owner_user_id` (nullable, unique-partial-indexed — 1 academy : 1 owner-user, resolved server-side from the caller's own JWT `sub`, never a client-supplied id); a new leaf table `academy_membership_requests` (pending/accepted/rejected, partial-unique-on-pending), deliberately kept separate from `academy_memberships` (which stays exactly active/left, untouched) — same one-table-per-concern convention as `academy_contact_requests`.
- New backend module `backend/src/modules/marketplace/academy-owner/` — ~15 endpoints under `/academy/me/*` (`@Roles('academy')`): profile CRUD, logo/cover/photo upload, stats, teachers active/pending/removed, accept/reject/remove, contact requests. Reuses `AcademiesModule`'s and `AcademyMembershipsModule`'s repositories (now exported for this purpose) rather than duplicating queries. Academy stats (teacher/student/batch counts) are derived from the same `BatchesRepository`/`BookingsService` methods §8's public academy page already used — no new batch/student ownership model was invented, per an explicit constraint in this feature's spec.
- Teacher-side join endpoints added to the existing `AcademiesController`/`AcademiesService` (`POST/GET /marketplace/academies/:slug/join-requests`, `GET .../me/join-requests`) — not a new module, since this is discovery-adjacent.
- Superadmin gets `POST /admin/academies/:id/owner` to create-or-link an owner account and grant the `academy` role — sign-*in* reuses the completely unmodified email-OTP flow (§6.7); only account *creation* is superadmin-gated, mirroring §8's existing academy-management boundary.
- Frontend: a 5th hand-rolled dashboard shell, `AcademyShell` (`web/src/components/academy-shell.tsx`), plus its own design-system trio (`web/src/components/academy/`) — following this codebase's established per-role-shell-duplication convention (Tutor/Student/Parent/Admin each already have their own) rather than a risky shared-shell refactor. New `/academy/*` route tree: overview (stats + pending requests + contact requests + reviews), profile editor, teachers (Active/Pending/Removed tabs), contact requests, reviews, photos, settings, and explicit "coming soon" empty states for batches/students (no ownership model invented there, per the same constraint above).
- Tutor-side `web/src/app/dashboard/find-an-academy/**` (copy of §8's student version, "Contact Academy" swapped for a "Request to Join" confirm-dialog flow with pending/declined state). Teacher Profile gained a "Teaching Arrangement" section — entirely derived from existing membership/request data, no new persisted field.

**Security — verified live, not just by code review**: created a second academy with its own owner account, then used that second owner's real JWT to call `POST /academy/me/teachers/{firstAcademysRequestId}/accept` — got a clean `403 "That request belongs to another academy"`, and `GET /academy/me` / `GET /academy/me/teachers/pending` with the second owner's token correctly returned only that academy's own (different) data, never the first academy's.

**Verified:** backend `tsc`/eslint/migration up-down-up round-trip/boot check (all new routes mapped, no module-cycle errors); frontend `tsc`/eslint/`next build` clean across all 53 routes; full live browser walkthrough — academy login → profile edit → confirmed reflected on the public academy page (§8) → teacher login → Teaching Arrangement → search → Request to Join (with message) → academy login → Teachers/Pending → Accept → notification delivered (`academy_join_accepted`) → teacher's "Teaching under" badge now clickable → academy Remove → teacher account/profile/verification-status/academy-list all confirmed intact except the membership → cross-academy security check above → mobile viewport (375px) confirmed no horizontal overflow on both the Academy Dashboard and the tutor-side Find an Academy page.

**Known limitations (intentionally deferred, stated explicitly in the feature's own spec — not oversights):** `/academy/batches` and `/academy/students` are empty "coming soon" states, since no academy-level batch/student ownership model exists in this codebase; a single `owner_user_id` per academy (no multi-admin-per-academy); no cooldown on re-requesting after a rejection.

## 10. Student Dashboard redesign (post-blueprint feature)

Built 2026-08-21, one session — commit `254262c`. Migrates the Student Portal (`web/src/app/student/**`) from its original horizontal top-nav-with-"More"-dropdown onto the same `PortalSidebar`-driven shell §9's Academy Dashboard and the earlier Teacher Dashboard redesign already share, then reworks the information architecture on top of it per a 20-section user spec. Shipped in 4 checkpointed stages (mirroring how the Teacher/Academy redesigns were delivered), each independently reviewed and verified before the next started.

**Stage A — Shell/nav migration.** New `student-nav.ts`/`student-sidebar.tsx` (thin config wrappers over the existing generic `PortalSidebar`, zero changes to that shared component), `student-shell.tsx` rebuilt on the `AcademyShell` structural template. Sidebar groups: Main (Overview) / Discover (Find a Teacher, Find an Academy) / My Learning (My Batches, Schedule, Assignments, Quizzes, Materials) / Progress (Attendance, Announcements, Doubts & Help), footer Profile-only. **No Settings nav item** — deliberately deferred per explicit user decision (no `/student/settings` page exists and none was speced); the footer array is structured so adding one later is a one-line change, not a scaffolding job.

**Stage B — Overview redesign, My Batches, Batch Workspace.** Overview's Next Class became its own card with a real empty state (previously buried inside the greeting hero); the 3 stat cards became clickable links; "What Needs Your Attention" lost an arbitrary 5-item cap; "Recent Updates" now merges materials with announcements, not just announcements. My Batches cards now link into a **net-new Batch Workspace** (`/student/batches/[id]/{overview,schedule,assignments,materials,quizzes,attendance,announcements,doubts}`) — a per-batch mini-site with its own header/tab bar. Built by extracting each global page's rendering into a shared presentational component (`ScheduleList`, `AssignmentsList`, `MaterialsList`, `QuizzesList`, `AttendanceDetail`, `AnnouncementsList`, `DoubtsPanel` — all under `web/src/components/student/`) so the global "all batches" pages and the workspace's "one batch" tabs render the exact same component with different data, instead of duplicating ~8 page implementations.

**Stage C — Discovery, Schedule, Assignments, Materials, Quizzes, Attendance polish.** Find a Teacher/Academy cards gained a verified badge, years-experience, languages, grade range, and batch count, plus a 5-mode (tutors)/3-mode (academies) sort — all derived from columns already selected or one column-add away, no new tables. Schedule gained Today/This Week/2 Weeks/Month tabs (driving `/sessions/upcoming`'s already-supported `from`/`to` params) plus a batch filter. Assignments now group into Due Today/Due Soon/Overdue (not just one flat "needs attention" bucket) plus a summary stats line. Materials group by a mime-derived category (PDFs/Documents/Images/Videos/Other — deliberately not "Notes/Worksheets/Links" as one literal reading of the spec suggested, since nothing in the data distinguishes those). Quizzes split into Available/Completed only, not a 3-way "In Progress" split — quizzes are single-attempt take-and-submit, so there's no real in-progress state to model without inventing one. Attendance gained a monthly calendar widget built from the already-fetched history, no new endpoint.

**Stage D — Profile enrichment, notification click-through, empty states.** Migration `0034_student_profile_enrichment` adds `subjects`/`languages`/`location`/`teaching_mode`/`learning_goals` to `profiles_student` (mirrors `0029`'s shape for `profiles_tutor`) — avatar/photo upload explicitly excluded, deferred as its own future piece of work. `NotificationsBell` gained an optional `resolveHref` prop (a notification `type` routes to a different page per portal, so the mapping can't live inside the shared component); only `StudentShell` passes one — Teacher/Academy/Parent shells are byte-for-byte unchanged. A shared `NoBatchesEmptyState` component (title + Find a Teacher/Find an Academy buttons) replaced three separately-worded "no batches" empty states (My Batches, Doubts, Quizzes).

**Real bugs found while building this:**
- `GET /attendance/summary/batch/:id` (a pre-existing, older endpoint — confirmed via its own doc comment in `web/src/lib/types.ts`) returns `attendanceRate`, not `rate` like every other attendance-summary shape in the app. The new Batch Workspace Attendance tab was the first caller to actually consume this route's response as an `AttendanceSummary`-shaped object, and silently rendered "undefined%" until caught by live browser testing and fixed with an explicit field-rename mapping in that one page.
- Next.js App Router's `layout.tsx` files only allow a fixed set of named exports (`default`, `metadata`, `generateStaticParams`, ...) — anything else fails `next build`'s route-type check with an opaque `OmitWithTag<...> does not satisfy the constraint '{ [x: string]: never }'` error, invisible to `tsc --noEmit`. The Batch Workspace's `useBatchWorkspace()` hook + React context were originally defined inline in `app/student/batches/[id]/layout.tsx`; had to be extracted into `web/src/components/student/batch-workspace-context.tsx` once `next build` (not just `tsc`) caught it.

**Verified:** `tsc --noEmit` and scoped `eslint` clean on both frontend and backend at every stage; full production `next build` (all 8 new Batch Workspace routes registered) and `nest build` clean; the `0034` migration round-tripped up→down→up against the local dev DB. Live browser-tested end to end against real seeded data at every stage — all 5 tutor / 3 academy sort modes return correctly-shaped 200s, every one of the 8 Batch Workspace tabs renders correctly (including the attendance-rate bug fix), a full profile save/reload round-trip persists all 5 new fields, and two real notifications (`quiz_published`, `submission_graded`) were clicked and confirmed to both mark-read and navigate to the correct quiz/assignment page.

**⚠ Migration `0034` has NOT been applied to production.** It was applied and round-tripped against the local dev DB only (see §5's "Production migration gate" pattern below — `git push` never runs migrations, Render's `CMD` is just `node dist/main`). `ProfilesRepository.upsertStudent` unconditionally references the 5 new columns in its `INSERT`/`ON CONFLICT` statement, so every `PUT /profiles/student` call in production will fail with a real Postgres "column does not exist" error until `npm run migrate:up` is run against the production `DATABASE_URL` (Render Shell tab) — this is the exact same class of outage §5 already documents for migrations `0024`–`0029`. Do this before considering Stage D live.

**Known follow-ups, not built (out of scope for this pass, not oversights):** a real `/student/settings` page (no content was ever speced for one); student profile photo upload (would mirror `profiles_tutor.avatar_object_key`'s existing presigned-upload pattern exactly, just not built here); Announcements read/unread state (no backend read-tracking exists for announcements at all — would need a new table, not a UI change) and a genuine quiz "in progress" state (would need the assessment schema to model partial attempts, which it currently doesn't) were both deliberately left as honest gaps rather than fabricated.
